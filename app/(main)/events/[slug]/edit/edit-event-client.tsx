...
export default function EditEventClient({ event, initialCategorySlugs }: EditEventClientProps) {
	...
	const initialStartTime = useMemo(
		() => tzDateStringToDateTimeStringWithNoTimezone(event.start_time, TIMEZONE),
		[event.start_time],
	);
								...
								<form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
									<div className="space-y-4 stagger-animation">
										...
										<FormField
											...
											name="categorySlugs"
											render={({ field }) => (
												<FormItem className="animate-in fade-in-0 slide-in-from-bottom-2 duration-300 delay-450">
					...
					<Card className="h-full transition-all duration-200">
						...
						<CardContent>
							<div className="space-y-4">
									...
									<div>
										...
										<p>
											{watchedValues.startTime
												? new Date(watchedValues.startTime).toLocaleString("pt-PT", {
														dateStyle: "medium",
														timeStyle: "short",
													})
												: "Not set"}
								...
								{(watchedValues.categorySlugs?.length ?? 0) > 0 && (
									<div className="animate-in fade-in-0 slide-in-from-bottom-2 duration-300 delay-500">
										<span className="font-medium text-muted-foreground text-sm">Categories:</span>
										<div className="mt-1 flex flex-wrap gap-1.5">
											{(watchedValues.categorySlugs ?? []).map((slug) => {
												const category = EVENT_CATEGORIES.find((c) => c.slug === slug);
												if (!category) return null;
												return (
													<span
														key={slug}
														className="inline-flex items-center rounded-full bg-[#DFF6F8] px-2.5 py-0.5 text-xs font-medium text-[#104357] dark:bg-[#04C9D8]/10 dark:text-[#E3F2F7]"
													>
														{category.name}
													</span>
												);
