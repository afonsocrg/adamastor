...
import { copyPost, deletePost, publishPost, unpublishPost } from "@/lib/posts";
import { Copy, Ellipsis, Eye, EyeOff, Globe, Loader2, Pencil, Trash2 } from "lucide-react";
...
export function PostActions({ post }: PostActionsProps) {
	...
	const handleAction = async (e: React.MouseEvent | Event, action: string, handler: (id: string) => Promise<void>) => {
		e.stopPropagation();
	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger asChild>
				<Button size="sm" variant="ghost" className="flex items-center gap-2 p-2 rounded-lg">
			...
			<DropdownMenuContent align="end">
				...
				<DropdownMenuItem asChild className="flex items-center gap-2 cursor-pointer">
					<Link href={`/dashboard/posts/${post.id}/edit`} className="flex items-center gap-2 cursor-pointer">
				...
				{post.is_public ? (
					...
					>
						{loadingAction === "publish" ? <Loader2 className="w-4 h-4 animate-spin" /> : <Globe className="w-4 h-4" />}{" "}
						Publish
					</DropdownMenuItem>
				)}
				<DropdownMenuItem
