		...
		<div className="space-y-4">
			{posts?.length === 0 ? (
				...
				posts?.map((post) => (
					<div
						key={post.id}
						className="bg-background border rounded-lg p-6 flex flex-col sm:flex-row justify-between items-center gap-4"
					>
						<div className="flex items-center gap-2 flex-1">
							<PublishedTag is_public={post.is_public} />
							<h2 className="text-xl font-semibold text-primary truncate text-wrap">{post.title}</h2>
						</div>
