...
import { ensureUniqueSlug, generateUniqueSlug, isValidSlug } from "@/lib/slugs";
...
export async function POST(request: Request, routeParams: RouteParams) {
	try {
		...
		const { data: existingPost } = await supabase.from("posts").select("id, slug").eq("id", id).single();
		...
		revalidatePostContent({ id: updatedPost.id, slug: updatedPost.slug });
		if (existingPost?.slug && existingPost.slug !== updatedPost.slug) {
			revalidatePostContent(existingPost);
		}
