...
function formatEventDay(date: string | Date): { primary: string; secondary: string } {
...
export default function HomeSidebar({ opinions, upcomingEvents }: HomeSidebarProps) {
	...
	return (
		<aside className="flex flex-col gap-8 lg:sticky lg:top-24 lg:gap-10">
			{opinions.length > 0 && (
				<section className="lg:rounded-lg lg:border lg:border-navy-frame lg:bg-white lg:p-6 lg:dark:border-cyan-glow/[0.18] lg:dark:bg-transparent">
				...
				<section className="lg:rounded-lg lg:border lg:border-navy-frame lg:bg-white lg:p-6 lg:dark:border-cyan-glow/[0.18] lg:dark:bg-transparent">
					...
					<ul className="mt-4 divide-y divide-navy-frame dark:divide-cyan-glow/[0.12]">
						{upcomingEvents.map((event) => {
							const { primary, secondary } = formatEventDay(event.start_time);
							...
							return (
								<li key={event.id}>
									<Link
										...
										className="group flex items-start gap-3 py-4 transition-colors duration-150 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
									>
										{/* Date plaque — tabular, calm, holds its width. */}
										<time
											dateTime={new Date(event.start_time).toISOString()}
											className="flex w-12 shrink-0 flex-col items-center rounded-md border border-navy-frame bg-navy-veil/40 px-1.5 py-1.5 text-center leading-none dark:border-cyan-glow/[0.18] dark:bg-cyan-glow/[0.04]"
										>
											<span className="text-[10px] font-semibold uppercase tracking-[0.14em] text-navy-tone dark:text-cyan-dim">
												{secondary}
											</span>
											<span className="mt-0.5 text-sm font-bold text-navy [font-variant-numeric:tabular-nums] dark:text-cyan-lifted">
												{primary}
											</span>
										</time>
										<div className="min-w-0 flex-1">
											...
											{event.city && (
												<p className="mt-1 text-xs text-muted-foreground">{formatCity(event.city)}</p>
											)}
										</div>
									</Link>
								</li>
